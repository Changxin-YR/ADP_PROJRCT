...
## 9. 智能体放开与自愈 — 2026-09-10 深夜（release 20260910-agentfix-r6）

用户要求"权限照旧锁死，其余别锁死、要更懂自然语言"。本轮放开与增强：

**放开的（都不是权限）**
- 单轮工具预算 6 → 12；两条"只准调用一次 / 不许用其它工具"改成提示（保留 resource 线索）。
- 只读分页上限 20 → 50，并明确允许"翻页拿全量"。
- 提示顺序：身份 → 【当前页面】 → 【最近对话】 → 行为契约 → 用户请求（前端每轮携带最近 8 条对话与当前路由，跨 worker/重启不丢上下文；不写浏览器存储）。
- 超时：后端 90s / nginx 150s / 上下文令牌 180s，长回合不再被 30s 掐断。

**更懂业务**
- 契约里补 operation + resource 词汇表：塘口/农场/片区/物料、投喂 feed-logs / 用药 medications / 巡塘 daily-operations / 出塘 harvests / 批次 batches、仓储 issues·receipts·transfers·alerts、采购销售成本待办等；
- 明确"resource 猜错就换名字或问用户，不要连续重试同一个错名"。

**可靠性（今晚实测踩到的两个真问题）**
- 侧车子进程缓存无上限（每个会话 namespace 一个 ~70MB 进程，永不回收）→ 新增 `harness_cache.HarnessCache`（LRU，默认 6/worker），实测 5 个并发会话稳定在 5 个子进程。
- 子进程坏掉后被永久复用（表现为连续 502「智能助手返回格式无效」）→ 协议/传输异常与格式异常都会丢弃缓存，下一轮自动换新进程。

**模型选型（同一套指令 + 完整契约实测）**
| 模型 | 表现 | 结论 |
| --- | --- | --- |
| qwen-plus（现用） | 7–30s，操作选择准确，回答质量好 | 保留 |
| qwen3.8-max | 单轮 16–39s（慢 2–3 倍） | 需要更强推理时可手动切 |
| deepseek-v4-pro | 工具调用在这条链路上失败（38–64s 后返回格式错误） | 不用 |

切换方式：`AGENT_MODEL=<模型名>` + `systemctl restart adp-next`。

**仍然保留的边界（说明理由）**
- 封闭操作清单（171 条、固定路径，不接受任意 URL/SQL）——这是"权限一致"能被强制的前提；
