...
function openOrder(row?: SalesOrder) { editingOrder.value = row ?? null; editingDelivery.value = null; correcting.value = null; dialogError.value = ''; setForm(orderFields.value, row as unknown as Record<string, unknown>); formOpen.value = true }
function openDelivery(row?: SalesDelivery, correction = false, order?: SalesOrder) {
  editingOrder.value = null; editingDelivery.value = correction ? null : row ?? null; correcting.value = correction ? row ?? null : null; correctionReason.value = ''; dialogError.value = ''
  setForm(deliveryFields.value, row as unknown as Record<string, unknown>); if (order) form.sales_order_id = order.id; formOpen.value = true
}
