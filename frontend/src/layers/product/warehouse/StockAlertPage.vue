...
async function saveHandling() {
  ...
  await run(async () => {
    try {
      ...
      if (actionCode.value === 'transfer' || actionCode.value === 'scrap' || actionCode.value === 'recheck') payload.resolution_document_id = Number(resolutionDocumentId.value)
