...
class MySqlReturnStore:
    ...
    @staticmethod
    def _joins(kind: str) -> tuple[str, str]:
        if kind == "purchase":
            return (
                " JOIN warehouse_documents d ON d.id=r.source_receipt_id"
                " JOIN warehouses w ON w.id=d.warehouse_id",
                "w",
            )
        return (
            " JOIN sales_deliveries d ON d.id=r.source_delivery_id"
            " JOIN sales_orders o ON o.id=d.sales_order_id",
            "o",
        )

    @classmethod
    def _require_scope(cls, user: dict[str, Any], row: dict[str, Any]) -> None:
        if not row_in_scope(user, row):
            raise DomainError("DATA_SCOPE_FORBIDDEN", "无权访问授权范围之外的退货记录", 403)
    ...
    def list_returns(self, kind: str, *, user: dict[str, Any], page: int = 1, page_size: int = 20, status: str | None = None, search: str | None = None, **_: Any) -> dict[str, Any]:
        ...
        clauses, values = [], []
        joins, scope_alias = self._joins(kind)
        scope, scoped = scope_predicate(user, scope_alias)
        if scope:
        ...
        with get_connection(self.settings) as connection, connection.cursor() as cursor:
            cursor.execute(f"SELECT COUNT(*) AS total FROM {table} r{joins} WHERE {where}", tuple(values)); total = int((cursor.fetchone() or {}).get("total", 0))
            cursor.execute(f"SELECT r.*,{scope_alias}.area_id,{scope_alias}.farm_id FROM {table} r{joins} WHERE {where} ORDER BY r.updated_at DESC,r.id DESC LIMIT %s OFFSET %s", tuple(values + [page_size, (page - 1) * page_size]))
    ...
    def get_return(self, kind: str, record_id: int, *, user: dict[str, Any]) -> dict[str, Any] | None:
        ...
        with get_connection(self.settings) as connection, connection.cursor() as cursor:
            joins, scope_alias = self._joins(kind)
            cursor.execute(f"SELECT r.*,{scope_alias}.area_id,{scope_alias}.farm_id FROM {table} r{joins} WHERE r.id=%s", (record_id,))
            row = cursor.fetchone()
            if row:
    ...
    def create_return(self, kind: str, payload: dict[str, Any], *, user: dict[str, Any], user_id: int) -> dict[str, Any]:
        ...
        with get_connection(self.settings) as connection, connection.cursor() as cursor:
            if kind == "purchase":
                ...
                self._require_scope(user, source)
                ...
                cursor.execute("SELECT d.*,r.id AS receivable_id,o.unit_price,o.organization_id,o.area_id,o.farm_id FROM sales_deliveries d JOIN sales_receivables r ON r.source_delivery_id=d.id JOIN sales_orders o ON o.id=d.sales_order_id WHERE d.id=%s AND d.status='verified' FOR UPDATE", (payload.get("source_delivery_id"),)); source = cursor.fetchone()
    ...
    def set_return_status(self, kind: str, record_id: int, status: str, *, expected_version: int, user: dict[str, Any], user_id: int, reason: str | None = None) -> dict[str, Any]:
        ...
        with get_connection(self.settings) as connection, connection.cursor() as cursor:
            joins, scope_alias = self._joins(kind)
            cursor.execute(f"SELECT r.*,{scope_alias}.area_id,{scope_alias}.farm_id FROM {table} r{joins} WHERE r.id=%s FOR UPDATE", (record_id,)); row = cursor.fetchone()
    ...
    def delete_return(self, kind: str, record_id: int, *, user: dict[str, Any], user_id: int) -> dict[str, Any]:
        ...
        with get_connection(self.settings) as connection, connection.cursor() as cursor:
            joins, scope_alias = self._joins(kind)
            cursor.execute(f"SELECT r.*,{scope_alias}.area_id,{scope_alias}.farm_id FROM {table} r{joins} WHERE r.id=%s AND r.status='draft' FOR UPDATE", (record_id,)); row = cursor.fetchone()
