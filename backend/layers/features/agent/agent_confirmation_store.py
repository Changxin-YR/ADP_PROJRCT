...
class MySqlAgentConfirmationStore(AgentConfirmationStore):
    ...
    def claim(self, *, token: str, user_id: int, session_hash: str, now: datetime | None = None) -> AgentConfirmation | None:
        ...
        return AgentConfirmation(id=int(row["id"]), idempotency_key=f"agent-confirmation:{row['id']}", token_hash=row["token_hash"], user_id=int(row["user_id"]), session_hash=row["session_hash"], conversation_id=row["conversation_id"], request_id=row["request_id"], tool_name=row["tool_name"], payload=json.loads(row["payload_json"] or "{}"), status=row["status"], expires_at=row["expires_at"], used_at=row["used_at"], created_at=row["created_at"])
