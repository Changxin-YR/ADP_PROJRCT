...
def validate_master_hierarchy(
    ...
    *,
    ...
    if not _exists(cursor, "SELECT id FROM organizations WHERE id=%s", (organization_id,)):
        raise DomainError("MASTER_SCOPE_MISMATCH", "企业不存在", 400)
    if farm_id and not _exists(cursor, "SELECT id FROM farms WHERE id=%s AND organization_id=%s", (farm_id, organization_id)):
        raise DomainError("MASTER_SCOPE_MISMATCH", "基地不属于所选企业", 400)
    if area_id and not _exists(
        cursor,
        "SELECT id FROM areas WHERE id=%s AND organization_id=%s AND farm_id=%s",
    ...
    if group_id and not _exists(
        cursor,
        "SELECT id FROM pond_groups WHERE id=%s AND organization_id=%s AND farm_id=%s AND area_id=%s",
    ...
    if supplier_id and not _exists(
        cursor,
        "SELECT id FROM business_partners WHERE id=%s AND organization_id=%s AND partner_type='supplier'",
