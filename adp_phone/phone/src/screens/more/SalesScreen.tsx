...
import { SalesOrder, DOCUMENT_STATUS_LABELS, DOCUMENT_STATUS_COLORS } from '../../types';
...
const SalesScreen: React.FC<SalesScreenProps> = ({ navigation }) => {
  ...
  const fetchOrders = useCallback(async (pageNum = 1, refresh = false) => {
    try {
      ...
      const data = await salesApi.listOrders(pageNum, 20);
  ...
  const renderItem = ({ item }: { item: SalesOrder }) => (
    <TouchableOpacity style={styles.orderCard} activeOpacity={0.7}>
      ...
      {item.species && (
        <Text style={styles.speciesRow}>{item.species} - {item.quantity} {item.unit}</Text>
      )}
      <View style={styles.orderFooter}>
        <Text style={styles.orderDate}>{item.sold_at?.split('T')[0] || item.created_at?.split('T')[0]}</Text>
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.title}>销售管理</Text>
          <View style={{ width: 36 }} />
        </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            <Ionicons name="cash-outline" size={48} color={colors.gray3} />
            <Text style={styles.emptyText}>暂无销售订单</Text>
          </View>
        }
      />
    </SafeAreaView>
