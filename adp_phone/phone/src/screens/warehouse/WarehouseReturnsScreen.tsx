...
const WarehouseReturnsScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const fetchData = useCallback(async (pageNum = 1, refresh = false) => {
    try {
      ...
      const data = await warehouseApi.listReturns(pageNum, 20);
    ...
    <Card style={styles.card}>
      <View style={styles.cardHeader}>
        <View style={styles.iconWrap}>
          <Ionicons name="return-down-back" size={20} color="#5AC8FA" />
        </View>
      ...
      <Text style={styles.nameText}>{item.name || '物料退库'}</Text>
    </Card>
  );
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>退库记录</Text>
        <View style={{ width: 36 }} />
      </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              <Ionicons name="return-down-back-outline" size={48} color={colors.gray3} />
              <Text style={styles.emptyText}>暂无退库记录</Text>
            </View>
          }
        />
      )}
...
const styles = StyleSheet.create({
  ...
  iconWrap: { width: 36, height: 36, borderRadius: borderRadius.md, backgroundColor: '#5AC8FA15', justifyContent: 'center', alignItems: 'center', marginRight: spacing.md },
...
export default WarehouseReturnsScreen;
