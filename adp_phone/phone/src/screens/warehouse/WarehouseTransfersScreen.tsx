...
const WarehouseTransfersScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const fetchData = useCallback(async (pageNum = 1, refresh = false) => {
    try {
      ...
      const data = await warehouseApi.listTransfers(pageNum, 20);
  ...
  const renderItem = ({ item }: { item: WarehouseDocument }) => (
    <Card style={styles.card}>
      ...
      <Text style={styles.nameText}>{item.name || '仓间调拨'}</Text>
    </Card>
  );
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>仓间调拨</Text>
        <View style={{ width: 36 }} />
      </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptyText}>暂无调拨记录</Text>
            </View>
          }
        />
      )}
...
const styles = StyleSheet.create({
  ...
  iconWrap: { width: 36, height: 36, borderRadius: borderRadius.md, backgroundColor: '#AF52DE15', justifyContent: 'center', alignItems: 'center', marginRight: spacing.md },
